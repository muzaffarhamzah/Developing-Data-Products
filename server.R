...
shinyServer(
        function(input, output) {
                ...
                output$prediction <- renderPrint({musicPredict(input$mood)})
                
                output$inputValue2 <- renderPrint({input$movie})
                output$prediction2 <- renderPrint({moviePredict(input$movie)})
        }
)